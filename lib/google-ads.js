...
exports.initializeAdsClient = initializeAdsClient;
exports.getCustomer = getCustomer;
exports.verifyConnection = verifyConnection;
exports.createSearchCampaign = createSearchCampaign;
exports.createAdGroupAndAds = createAdGroupAndAds;
const google_ads_api_1 = require("google-ads-api");
// Initialize the Google Ads API client
// This uses the credentials we placed in .env.local
let client = null;
function initializeAdsClient() {
    try {
 ...
 */
async function createSearchCampaign(name, budgetAmountMicros = 5000000) {
    ...
    try {
        ...
        const campaignResponse = await customer.campaigns.create([{
                name: `${name} #${Date.now()}`,
                advertising_channel_type: google_ads_api_1.enums.AdvertisingChannelType.SEARCH,
                status: google_ads_api_1.enums.CampaignStatus.PAUSED, // Keep PAUSED so it doesn't spend money immediately
                manual_cpc: {},
                contains_eu_political_advertising: google_ads_api_1.enums.EuPoliticalAdvertisingStatus.DOES_NOT_CONTAIN_EU_POLITICAL_ADVERTISING,
 ...
 */
async function createAdGroupAndAds(campaignResourceName, adGroupName, adContent) {
    const customer = getCustomer();
    try {
        // 1. Create Ad Group
        const adGroupResponse = await customer.adGroups.create([{
                ...
                status: google_ads_api_1.enums.AdGroupStatus.ENABLED,
                type: google_ads_api_1.enums.AdGroupType.SEARCH_STANDARD,
